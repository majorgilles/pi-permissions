...
In `ask` mode, `write` and `edit` no longer open editable review buffers. Instead they show a read-only Claude-Code-style diff with transparent-style red backgrounds for deletions, transparent-style green backgrounds for additions, and syntax highlighting for recognized source-code file types.
